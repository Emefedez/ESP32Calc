...
math request queue
        ...
        v
math worker
  - lightweight numeric/exact engine
  - optional Giac CAS bridge
...
3. A CAS bridge interface that can later be backed by Giac.
...
3. Phase 2: introduce a display-neutral math AST and renderer model for the
   e-paper canvas.
4. Phase 3: add a CAS bridge interface. Keep a local fallback engine for common
   operations.
5. Phase 4: evaluate Giac import or clean integration, including license,
   flash size, PSRAM use, exceptions, and stack size.
