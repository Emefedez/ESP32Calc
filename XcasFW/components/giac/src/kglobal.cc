...
extern "C" void Sleep(unsigned int miliSecond);
...
#elif defined(GIAC_ESP32_SILENT_IO)
  static stdostream * _logptr_=0;
#else
  static stdostream * _logptr_=&cout;
#endif
...
#ifdef GIAC_ESP32_SILENT_IO
      return _logptr_?_logptr_:&null_ostream();
#else
      return _logptr_;
#endif
  }
  ...
  void ctrl_c_signal_handler(int signum){
    ctrl_c=true;
#if !defined NSPIRE_NEWLIB && !defined WIN32 && !defined(_WIN32) && !defined BESTA_OS && !defined NSPIRE && !defined FXCG && !defined TICE && !defined HAVE_NO_SIGNAL_H
    if (child_id)
