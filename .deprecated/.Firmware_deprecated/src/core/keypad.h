...
// Scan matrix: pull each row LOW, read columns, debounce, return first new press or KEY_NONE
key_code_t keypad_scan(void);
// True if ALPHA toggle is active (toggled by KEY_ALPHA, consumed in scan)
bool keypad_alpha_active(void);
// True if SHIFT toggle is active (auto-released after any non-modifier key)
